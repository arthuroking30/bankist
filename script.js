...
const calcDate = function (date, locale) {
  const days = Math.trunc((Date.now() - new Date(date)) / 86400000);
...
const formatCur = function (number, acc) {
  return number.toLocaleString(acc.locale, {
    style: 'currency',
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const displayDate = calcDate(acc.movementsDates[i], acc.locale);
...
labelDate.textContent = `${new Date().toLocaleDateString()}, ${new Date()
  .toLocaleTimeString()
  .slice(0, -3)}`;
...
// setTimeout(() => console.log('print after 5 sec'), 5000);
const startClock = function () {
  ...
  const tick = function () {
    ...
    if (time === 0) {
      ...
      // Hide UI
      containerApp.style.opacity = 0;
      labelWelcome.textContent = 'Session timed out, Log in to get started';
    }
    time--;
  };
  tick();
